...
export const CadastroReligiosoPublico: React.FC = () => {
  ...
  return <PublicShell><form onSubmit={submit} className="w-full max-w-6xl rounded-2xl bg-white/95 p-6 shadow-premium sm:p-9">
    ...
    <div className="mb-8 overflow-x-auto pb-2"><div className="flex min-w-[760px] items-start justify-between"><div className="relative flex w-full items-start justify-between">{etapas.map((label, index) => { const stepIndex = index + 1; const complete = step > stepIndex; const active = step === stepIndex; return <React.Fragment key={label}><button type="button" onClick={() => setStep(stepIndex)} title={label} className="relative z-10 flex w-20 shrink-0 flex-col items-center gap-2 text-center"><span className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold transition-colors ${complete ? 'bg-teal-500 text-white' : active ? 'bg-teal-500 text-white ring-4 ring-teal-500/15' : 'bg-slate-100 text-slate-400'}`}>{complete ? <CheckCircle2 className="h-4 w-4" /> : stepIndex}</span><span className={`text-[10px] font-semibold leading-tight ${active ? 'text-teal-700' : complete ? 'text-teal-600' : 'text-slate-400'}`}>{label}</span></button>{index < etapas.length - 1 && <span className={`mt-4 h-px min-w-2 flex-1 ${complete ? 'bg-teal-500' : 'bg-slate-200'}`} />}</React.Fragment> })}</div></div></div>
