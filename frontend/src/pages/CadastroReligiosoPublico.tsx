...
export const CadastroReligiosoPublico: React.FC<CadastroReligiosoPublicoProps> = ({ adminMode = false }) => {
  ...
  useEffect(() => {
    const load = async () => {
      const [{ data: obrasData }, { data: religiosoData }] = await Promise.all([
  ...
  const validateStep = () => {
    ...
    if (step === 13 && (!documentos.length || !base.consentimento_dados)) return 'Anexe pelo menos um documento e confirme a autorização.';
    return null;
  };
  ...
  return <PublicShell><form onSubmit={submit} className="w-full max-w-6xl rounded-2xl bg-white/95 p-6 shadow-premium sm:p-9">
    <header className="mb-8 text-center"><h1 className="font-serif text-3xl font-bold text-primary">{adminMode ? (religiosoId ? 'Editar Religioso' : 'Novo Religioso') : 'Atualização de Dados dos Religiosos'}</h1><p className="mt-1 text-xs uppercase tracking-widest text-slate-400">Província BRM</p></header>
