...
interface ObraReferencia { id: string; nome: string; localidade: string | null; uf: string | null }
...
const defaultPublicConfig: ReligiososConfigPublica = { ativo: true, titulo: 'Atualização de Dados dos Religiosos', mensagem_abertura: '', mensagem_fechamento: 'As inscrições estão temporariamente fechadas.', mensagem_confirmacao: 'Recebemos seus dados e documentos. A secretaria fará a conferência.', termos: 'Autorizo o uso dos dados pela Província BRM para atualização cadastral, gestão institucional e contato pastoral/administrativo.', exigir_documentos: true, instrucoes_documentos: '' };
...
export const CadastroReligiosoPublico: React.FC<CadastroReligiosoPublicoProps> = ({ adminMode = false }) => {
  ...
  useEffect(() => {
    const load = async () => {
      const [{ data: obrasData }, { data: religiosoData }, { data: configData }] = await Promise.all([
        supabase.from('religiosos_obras_referencia').select('id,nome,localidade,uf').eq('status', 'Ativa').order('nome'),
  ...
  const validateStep = () => {
    const identification = ['grau', 'nome_civil', 'nome_religioso', 'data_nascimento', 'local_nascimento', 'municipio_nascimento', 'estado_nascimento', 'pais_nascimento', 'nacionalidade', 'cpf', 'rg', 'rg_orgao_expedidor', 'rg_data_emissao', 'titulo_eleitor', 'pis', 'cnh', 'cnh_categoria', 'passaporte'];
    ...
    if (step === 1 && !hasValues(base, identification)) return 'Preencha todos os campos de identificação.';
  ...
  if (!adminMode && !publicConfig.ativo) return <PublicShell><div className="max-w-xl rounded-2xl bg-white p-10 text-center shadow-premium"><h1 className="font-serif text-2xl font-bold text-primary">Inscrições fechadas</h1><p className="mt-3 text-sm text-slate-500">{publicConfig.mensagem_fechamento}</p></div></PublicShell>;
  if (success) return <PublicShell><div className="max-w-xl rounded-2xl bg-white p-10 text-center shadow-premium"><CheckCircle2 className="mx-auto mb-4 h-14 w-14 text-emerald-500" /><h1 className="font-serif text-2xl font-bold">Cadastro enviado</h1><p className="mt-3 text-sm text-slate-500">{publicConfig.mensagem_confirmacao}</p></div></PublicShell>;

  return <PublicShell><form onSubmit={submit} className="w-full max-w-6xl rounded-2xl bg-white/95 p-6 shadow-premium sm:p-9">
    <header className="mb-8 text-center"><h1 className="font-serif text-3xl font-bold text-primary">{adminMode ? (religiosoId ? 'Editar Religioso' : 'Novo Religioso') : publicConfig.titulo}</h1><p className="mt-1 text-xs uppercase tracking-widest text-slate-400">Província BRM</p>{!adminMode && publicConfig.mensagem_abertura && <p className="mx-auto mt-3 max-w-2xl text-sm text-slate-500">{publicConfig.mensagem_abertura}</p>}</header>
