...
export const Inicio: React.FC = () => {
  ...
  return <div className="space-y-6 animate-fade-in">
    <header className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-secondary">Visão geral</p><h1 className="mt-2 font-serif text-3xl font-bold text-primary">Olá, {user?.nome || 'Usuário'}</h1><p className="mt-2 max-w-2xl text-sm text-slate-500">Acompanhe as entradas, pendências e cadastros que precisam da atenção da secretaria.</p></div><div className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 text-xs font-semibold ${data.inscricoesAbertas ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-700' : 'border-amber-500/20 bg-amber-500/10 text-amber-700'}`}><span className={`h-2 w-2 rounded-full ${data.inscricoesAbertas ? 'bg-emerald-500' : 'bg-amber-500'}`} />Inscrições de religiosos {data.inscricoesAbertas ? 'abertas' : 'fechadas'}</div></header>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
      <Metric label="Religiosos" value={data.religiosos.total} icon={Users} tone="navy" href="/religiosos" />
      <Metric label="Em revisão" value={data.religiosos.revisao} icon={FileCheck2} tone="amber" href="/religiosos" />
      <Metric label="Hospedagens" value={data.hospedaria.total} icon={Hotel} tone="teal" href="/hospedagens-inscricoes" />
      <Metric label="Check-ins pendentes" value={data.hospedaria.checkins} icon={CalendarDays} tone="amber" href="/hospedagens-inscricoes" />
      <Metric label="Check-outs pendentes" value={data.hospedaria.checkouts} icon={CheckCircle2} tone="rose" href="/hospedagens-inscricoes" />
      <Metric label="Quartos ocupados" value={data.hospedaria.ocupados} icon={Building2} tone="green" href="/hospedagens-inscricoes" />
    </section>
    <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
      <Panel title="Cadastros de religiosos" subtitle="Entradas mais recentes do site público e do painel." action="Ver inscritos" href="/religiosos"><div className="divide-y divide-slate-100 dark:divide-slate-800">{data.religiosos.recentes.length ? data.religiosos.recentes.map(item => <div className="flex items-center justify-between gap-4 py-3" key={item.id}><div className="flex min-w-0 items-center gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><UserRound className="h-4 w-4" /></span><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-800 dark:text-slate-100">{item.nome}</p><p className="text-[11px] text-slate-400">{item.grau} · {item.origem}</p></div></div><span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-bold ${item.status === 'Em revisão' ? 'bg-amber-500/10 text-amber-700' : 'bg-emerald-500/10 text-emerald-700'}`}>{item.status}</span></div>) : <Empty text="Nenhum cadastro de religioso encontrado." />}</div></Panel>
      <Panel title="Próximas movimentações" subtitle="Inscrições recentes da hospedaria." action="Ver hospedaria" href="/hospedagens-inscricoes"><div className="divide-y divide-slate-100 dark:divide-slate-800">{data.hospedaria.recentes.length ? data.hospedaria.recentes.map(item => <div className="flex items-center justify-between gap-4 py-3" key={item.id}><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-800 dark:text-slate-100">{item.nome}</p><p className="text-[11px] text-slate-400">{item.cidade} · {dateLabel(item.chegada)}</p></div><span className="shrink-0 rounded-full bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300">{item.status}</span></div>) : <Empty text="Nenhuma hospedagem encontrada." />}</div></Panel>
    </section>
    <section className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
      <Panel title="Ações rápidas" subtitle="Atalhos para o trabalho da secretaria."><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1"><QuickAction href="/religiosos/novo" icon={Plus} title="Cadastrar religioso" text="Abrir ficha completa" /><QuickAction href="/religiosos-configuracoes" icon={Settings2} title="Configurar inscrições" text="Mensagens, termos e abertura" /><QuickAction href="/hospedagens-inscricoes" icon={ClipboardList} title="Revisar hospedagens" text="Pesquisar inscrições recebidas" /></div></Panel>
      <Panel title="Pendências da secretaria" subtitle="O que merece atenção agora."><div className="grid gap-3 sm:grid-cols-3"><Attention label="Religiosos aguardando revisão" value={data.religiosos.revisao} href="/religiosos" tone="amber" /><Attention label="Check-ins sem registro" value={data.hospedaria.checkins} href="/hospedagens-inscricoes" tone="rose" /><Attention label="Religiosos aprovados" value={data.religiosos.aprovados} href="/religiosos" tone="green" /></div></Panel>
    </section>
  </div>;
};
